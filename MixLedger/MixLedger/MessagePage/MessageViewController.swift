    ...
    */
    ...
    func agare(index: IndexPath){
        ...
        if let data = data?.inviteCard?[index.row]{
            firebaseManager.postRespondToInvitation(respond: true, accountID: data.accountID, accountName: data.accountName, inviterID: data.inviterID, inviterName: data.inviterName){resulr in
             print("--")
    ...
    func reject(index: IndexPath){
        print("agare in table \(index)")
        if let data = data?.inviteCard?[index.row]{
            firebaseManager.postRespondToInvitation(respond: false, accountID: data.accountID, accountName: data.accountName, inviterID: data.inviterID, inviterName: data.inviterName){resulr in
             print("--")
                self.data = self.saveData.myInfo
                self.tableView.reloadData()
            }
